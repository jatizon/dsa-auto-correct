...
def main():
    if len(sys.argv) != 5:
        print("Uso: python3 valgrind_debug.py <lab_root> <folder_alunos> <nome_aluno> <testcase>")
        sys.exit(1)

    lab_root = sys.argv[1]             # ex: lab5
    folder_alunos = sys.argv[2]        # ex: labs-alunos-t2
    aluno = sys.argv[3]                # ex: Joao_Da_Silva
    testcase = sys.argv[4]             # ex: casos_erro

    folder_aluno = os.path.join(lab_root, folder_alunos, aluno)
    ...
    # pasta do testcase
    testcase_dir = os.path.join(lab_root, "testcases", testcase)
